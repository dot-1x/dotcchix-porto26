...
import { type SrCharacter } from "@/lib/star-rail-api"
import {
  ELEMENT_COLORS,
  normalizeElement,
  type Element,
} from "@/lib/star-rail-data"
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
export function CharacterShowcase({
  ...
  const { counts, elements } = useMemo(() => {
    ...
    for (const c of characters) {
      ...
      if (el) map.set(el, (map.get(el) ?? 0) + 1)
    }
  ...
  const filtered =
    active === "All"
      ? characters
      : characters.filter((c) => normalizeElement(c.element) === active)
